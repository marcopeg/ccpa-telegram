...
Add support for OpenAI "codex" CLI engine:
...
Codex works very similarly to Copilot for session management (`--continue` flag) but you have to research the codex CLI for other params.
...
Options:
  -c, --config <key=value>
          Override a configuration value that would otherwise be loaded from `~/.codex/config.toml`. Use a dotted path (`foo.bar.baz`) to override nested values. The `value` portion is parsed as TOML. If it fails to parse as TOML, the raw string
  ...
  -a, --ask-for-approval <APPROVAL_POLICY>
          ...
          - on-failure: DEPRECATED: Run all commands without asking for user approval. Only asks for approval if a command fails to execute, in which case it will escalate to the user to ask for un-sandboxed execution. Prefer `on-request` for
