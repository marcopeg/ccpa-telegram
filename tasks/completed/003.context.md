...
| `project.name` | Config | Project name (falls back to internal slug if not set) |
| `project.cwd` | Config | Resolved absolute project working directory |
| `project.slug` | Config | Claude Code-compatible slug (full path, `/` → `-`) |
