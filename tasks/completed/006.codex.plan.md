...
> **Done**: `src/claude/` deleted; all logic lives in `src/engine/adapters/claude.ts`. Shared prompt-building (context resolution + downloads path) extracted to `src/engine/prompt.ts` and called by all adapters.
...
| `src/claude/executor.ts` | **Deleted** — migrated to engine adapter | 1.2 |
| `src/claude/parser.ts` | **Deleted** — migrated to engine adapter | 1.2 |
| `src/engine/prompt.ts` | **New** — shared contextual prompt builder | 1.2 |
| `src/engine/index.ts` | **New** — barrel export | 1.1 |
...
**Completed**: All 10 steps of Phase 1 are implemented. Build and lint pass cleanly.

**Key decisions during implementation:**
- Created `src/engine/prompt.ts` with `buildContextualPrompt()` as a shared helper for context resolution + downloads path injection, avoiding duplication across all 4 adapters.
- `src/engine/index.ts` barrel re-exports all public types/functions from the engine module.
- `EngineName` type is defined independently in both `src/config.ts` (as Zod infer) and `src/engine/types.ts` (as string literal union) to avoid circular imports between config → engine → types → config.
- Renamed `claudeSlug()` → `deriveProjectSlug()` in context resolver to avoid shadowing the `projectSlug` parameter.
- CLI `--engine <name>` flag defaults to `"claude"`, is validated against the enum, and feeds into both `runInit()` (config template + instructions file scaffolding) and `runStart()` (engine instantiation on `ProjectContext`).
- All handlers use `ctx.engine.execute()` / `ctx.engine.parse()` — no direct Claude imports remain in the handler layer.
