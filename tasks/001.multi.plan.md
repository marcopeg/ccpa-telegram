...
> Status legend: ⬜ pending · 🔄 in progress · ✅ done · ❌ blocked · 📝 notes
...
```
config.ts        ← needs full rewrite (new schema + resolver)
logger.ts        ← needs factory (flow/persist support)
types.ts         ← NEW: ProjectContext interface
    ↓
executor.ts      ← accept ProjectContext
whisper.ts       ← accept ProjectContext
fileSender.ts    ← accept ProjectContext (logger)
user/setup.ts    ← no changes (pure path utils)
telegram/chunker.ts ← no changes
    ↓
middleware/auth.ts       ← factory function
middleware/rateLimit.ts  ← factory function (per-bot Map)
handlers/text.ts         ← factory or context param
handlers/photo.ts        ← factory or context param
handlers/document.ts     ← factory or context param
handlers/voice.ts        ← factory or context param
commands/clear.ts        ← factory or context param
commands/start.ts        ← no changes
commands/help.ts         ← no changes
    ↓
bot.ts           ← accept ProjectContext, wire factories
    ↓
cli.ts           ← load multi-config, validate, Promise.all
...
## Phase 1 — Config: New Schema & Resolver
...
2. **`resolveProjectConfig(project, globals, configDir): ResolvedProjectConfig`**
   ...
   - Resolves `cwd` (relative → absolute from `configDir`)
...
4. **`deriveSlug(name?, cwd): string`**
   ...
   - Examples: `./foo/bar` → `foo-bar`, `/abs/path/proj` → `abs-path-proj`

5. **`resolveDataDir(dataDirRaw, projectCwd, configDir, slug): string`**
   - Empty/undefined → `<projectCwd>/.ccpa/users`
   - `~` → `<configDir>/.ccpa/<slug>/data`
   - Absolute path → used as-is
   - Relative path → `path.resolve(projectCwd, dataDirRaw)`
...
> 📝 Log files use the date at process startup. No within-process daily rotation — acceptable given typical usage patterns. Can be enhanced later.

---

## Phase 3 — ProjectContext Type
...
## Phase 4 — Services: Accept ProjectContext

**Status:** ⬜ pending

### `src/claude/executor.ts`
- `executeClaudeQuery(options, ctx: ProjectContext)` — replace `getConfig()` / `getLogger()` calls with `ctx.config` / `ctx.logger`

### `src/transcription/whisper.ts`
- `transcribeAudio(audioPath, ctx: ProjectContext)` — replace singletons
- Note: `config.transcription` can be undefined; handle gracefully

### `src/telegram/fileSender.ts`
- `sendDownloadFiles(gramCtx, userDir, ctx: ProjectContext)` — replace `getLogger()`

---

## Phase 5 — Middleware: Factory Functions
...
> 📝 Returning a cleanup function is important so that the `setInterval` is cleared on graceful shutdown, avoiding resource leaks in the multi-bot process.

---

## Phase 6 — Handlers & Commands: Factory Pattern
...
### Files affected
- `src/bot/handlers/text.ts` → `createTextHandler(ctx)`
- `src/bot/handlers/photo.ts` → `createPhotoHandler(ctx)`
- `src/bot/handlers/document.ts` → `createDocumentHandler(ctx)`
- `src/bot/handlers/voice.ts` → `createVoiceHandler(ctx)`
- `src/bot/commands/clear.ts` → `createClearHandler(ctx)`
- `src/bot/commands/start.ts` — no changes (no config dependency)
- `src/bot/commands/help.ts` — no changes (no config dependency)
- `src/bot/handlers/index.ts` — update re-exports

---

## Phase 7 — Bot: Factory Function
...
## Phase 8 — CLI: Multi-Bot Orchestration

**Status:** ⬜ pending

### `src/cli.ts` — `runStart(cwd)`

```
1. createStartupLogger()
2. loadMultiConfig(cwd) → { globals, projects }
3. validateProjects(projects) → abort on error
4. For each project:
   a. resolveProjectConfig(project, globals, cwd) → ResolvedProjectConfig
   b. Load .env from project.cwd (if exists)
5. createProjectLogger(config) for each project → logger
...
8. Promise.all(contexts.map(ctx => startBot(ctx)))
   - If any rejects → log error via startupLogger → process.exit(1)
9. Graceful shutdown: on SIGINT/SIGTERM, run all shutdown fns
```

### `src/cli.ts` — `runInit(cwd)`
...
## Phase 9 — Exports: Update index.ts
...
## Phase 10 — README: Full Rewrite

**Status:** ⬜ pending

### Sections to rewrite / add

1. **Features** — add "Multi-project support: run multiple bots from one config"
2. **How It Works** — update for multi-project; clarify config file location vs. project CWD
3. **Quick Start** — update commands for new config format
4. **Configuration** — complete overhaul:
...
5. **Directory Structure** — show multi-project layout:
   ```
   workspace/
   ├── ccpa.config.json
   ├── .ccpa/
   ...
   ├── project-a/
   │   ├── CLAUDE.md
   │   └── .ccpa/users/    (default dataDir)
   └── project-b/
       └── CLAUDE.md
   ```
6. **CLI Commands** — update examples
7. **Environment Variables** — note that env var overrides are removed in v2; config file is source of truth
8. **Migration Guide** — brief section explaining the old format is no longer supported and how to migrate
...
Grammy's `bot.start()` returns a Promise that resolves only on shutdown. So `Promise.all([startBot(a), startBot(b)])` means both are waiting for shutdown — this is correct behavior for a long-running process.

### Risk: Graceful shutdown with multiple bots
The current code calls `process.exit(0)` in the shutdown handler. With multiple bots, all shutdown functions must be awaited before exit. Need to collect them from `Promise.all` — but `bot.start()` doesn't return until shutdown is requested. Pattern: `bot.start()` call is wrapped; the `shutdown()` function registered with signals calls `bot.stop()` externally, and the `bot.start()` Promise resolves.

> 📝 Grammy's `bot.stop()` can be called from outside; `bot.start()` resolves after stop. So the CLI can hold refs to all `bot` instances and call `bot.stop()` on each from the signal handler.
...
Multiple `.env` files may define the same env var keys. Loading them sequentially could cause later projects' env vars to override earlier ones at the `process.env` level. Since we're dropping env var overrides in the new format, this only matters if the Claude CLI subprocess reads env vars. The `.env` files can still be loaded per project to set env vars that the Claude subprocess will inherit — load them just before spawning the subprocess (in executor.ts) rather than at startup. For now, retain the existing "load at init time" behavior but document the limitation.

### Risk: `transcription` optional in project
If `transcription` is undefined (neither in project nor globals), the voice handler must handle this gracefully — either disable voice handling or send an error reply. Current code already handles `config.transcription?.model`, so this is a minor check.
...
1. Phase 1 — Config schema & resolver (foundation for everything else)
2. Phase 2 — Logger factory
3. Phase 3 — `types.ts`
4. Phase 4 — Services (executor, whisper, fileSender)
5. Phase 5 — Middleware factories
6. Phase 6 — Handler/command factories
7. Phase 7 — `bot.ts` refactor
8. Phase 8 — `cli.ts` orchestration
9. Phase 9 — `index.ts` exports
10. Phase 10 — `README.md`
11. Run `pnpm run lint:fix` and `pnpm run build` — fix any type errors
