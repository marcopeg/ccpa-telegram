...
- [ ] **1.1** Add a `loadEnvFiles(configDir: string, projectCwds: string[]): EnvSources` function in `src/config.ts`.
...
- [ ] **2.1** Add a `substituteEnvVars(obj: unknown, env: Record<string, string>, path?: string): unknown` recursive function.
...
- [ ] **3.1** Define `LocalConfigFileSchema` in `src/config.ts` — a fully partial variant of `MultiConfigFileSchema` where every field (including nested ones) is optional. The `projects` array items only require a `name` or `cwd` key for matching.

- [ ] **3.2** Add a `loadLocalConfig(configDir: string): LocalConfigFile | null` function.
  - Returns `null` if `ccpa.config.local.json` does not exist.
  - On parse or schema error: exits with a clear message.

- [ ] **3.3** Add a `deepMerge(base: object, override: object): object` utility (pure function, no external deps).
  - Plain objects → recurse.
  - Arrays → override replaces base (no concat).
  - Scalar / `undefined` → override wins if defined.

- [ ] **3.4** Add a `mergeLocalIntoBase(base: MultiConfigFile, local: LocalConfigFile): MultiConfigFile` function.
...
- [ ] **4.1** Change `loadMultiConfig` signature to also return `{ loadedFiles: string[] }` alongside `MultiConfigFile` (or return a combined object).

- [ ] **4.2** New execution order inside `loadMultiConfig`:
...
- [ ] **4.3** Update call sites in `src/cli.ts` to destructure the new return shape.
...
- [ ] **5.1** In `src/cli.ts` → `runStart`, after calling `loadMultiConfig`, emit a structured log:
...
- [ ] **6.1** Verify `.gitignore` already has `**/ccpa.config.local.json` and `**/.env.local`. Add any missing entries.
...
- [ ] **7.1** Update `examples/ccpa.config.json` so the `obsidian` project uses `${OBSIDIAN_TELEGRAM_TOKEN}` (already does) and the `timetracker` project uses `${TIMETRACKER_TELEGRAM_TOKEN}` instead of a hard-coded empty string.
- [ ] **7.2** Update `examples/ccpa.config.local.json` to show a minimal local override (only secrets, no `cwd` duplication).
- [ ] **7.3** Update `examples/.env` (or add `examples/.env.example`) to list both token variables.
...
- [ ] **8.2** Run `pnpm run typecheck` — fix any TypeScript errors.
