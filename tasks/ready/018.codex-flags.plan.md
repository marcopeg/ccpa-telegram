...
1. **Add `CodexEngineConfigSchema` (src/config.ts)**
...
2. **Extend `ResolvedProjectConfig` interface (src/config.ts)**
...
3. **Resolve codex flags in `resolveProjectConfig()` (src/config.ts)**
...
1. **Read codex config from `ctx.config` in `execute()` (src/engine/adapters/codex.ts)**
...
2. **Implement tier-based flag computation (src/engine/adapters/codex.ts)**
...
3. **Add startup logging (src/engine/adapters/codex.ts)**
...
1. **Update engine config table in README**
...
2. **Update the Codex engine section in README (around line 478–483)**
...
3. **Update globals config table in README (around line 270–276)**
   - Add `globals.engine.codex.*` entries referencing the codex flags.

4. **Update per-project config table in README (around line 297–303)**
   - Add `engine.codex.*` entries.

5. **Update example config (examples/hal.config.json)**
