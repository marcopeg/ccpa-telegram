...
- [ ] **1.1** Add `context: z.record(z.string(), z.string()).optional()` to `GlobalsFileSchema`, `ProjectFileSchema`, and `LocalProjectSchema` in `src/config.ts`.

- [ ] **1.2** Add `context: Record<string, string> | undefined` to `ResolvedProjectConfig`. In `resolveProjectConfig()`, merge globals context + project context (project wins) using spread: `{ ...globals.context, ...project.context }`. Store `undefined` if neither defines it.

- [ ] **1.3** Update the `ccpa init` template in `src/cli.ts` to include a commented-out example `context` block in the generated `ccpa.config.json`.
...
- [ ] **2.1** Create `src/context/resolver.ts` with the context resolution module. Export types:
  ```ts
...
- [ ] **2.2** Implement `evaluateBootTimeShells(context: Record<string, string>): Record<string, string>` — scans all context values for `#{...}` patterns, runs each via `execSync`, caches and replaces inline. Called once at startup. On failure, log warning and substitute empty string.

- [ ] **2.3** Implement `evaluateMessageTimeShells(value: string): string` — replaces `@{...}` patterns in a single string value by running the command via `execSync` at call time. On failure, log warning and substitute empty string.

- [ ] **2.4** Implement `resolveAppVars(value: string, vars: Record<string, string>): string` — replaces `${expression}` patterns by looking up `expression` in the provided variable map (which includes `bot.*`, `sys.*`, and env vars). This is distinct from the config-level env substitution (Phase 4 of task 002) which runs before Zod validation; this one runs at message time on context values only.
...
- [ ] **3.1** Implement `buildImplicitContext(gramCtx: GrammyContext): Record<string, string>` in `src/context/resolver.ts`. Extracts all `bot.*` keys from the Grammy context object:
  - `bot.messageId`, `bot.timestamp`, `bot.datetime`, `bot.userId`, `bot.username`, `bot.firstName`, `bot.chatId`, `bot.messageType`

- [ ] **3.2** Implement `buildSystemContext(): Record<string, string>` for `sys.*` keys:
  - `sys.datetime`, `sys.date`, `sys.time`, `sys.ts`, `sys.tz`
  - All evaluated fresh per call.

- [ ] **3.3** Implement `resolveContext(options: ResolveContextOptions): Promise<Record<string, string>>` — the main orchestrator:
...
- [ ] **3.4** Implement `formatContextPrompt(context: Record<string, string>, userMessage: string): string` — formats the final prompt:
  ```
  # Context
  - key1: value1
  - key2: value2
...
- [ ] **4.1** Implement `loadAndRunHook(hookPath: string, context: Record<string, string>, logger: Logger): Promise<Record<string, string>>` in `src/context/resolver.ts`.
...
- [ ] **4.2** Integrate both hooks into `resolveContext()` in chain order:
...
- [ ] **5.1** Add `bootContext: BootContext` to `ProjectContext` interface in `src/types.ts`. The `BootContext` type only contains `shellCache` (no hook reference — hooks are hot-reloaded per message). Also add `configDir: string` to `ResolvedProjectConfig` so the executor can locate the global hook path.

- [ ] **5.2** In `src/cli.ts` → `runStart`, after `resolveProjectConfig()`:
...
- [ ] **6.1** Add `gramCtx?: GrammyContext` to `ExecuteOptions` in `src/claude/executor.ts`.

- [ ] **6.2** In `executeClaudeQuery()`, if `gramCtx` is provided:
...
- [ ] **6.3** Update all four handlers (`text.ts`, `photo.ts`, `document.ts`, `voice.ts`) to pass `gramCtx` in `ExecuteOptions` when calling `executeClaudeQuery`.
...
- [ ] **7.1** Update `examples/ccpa.config.json` to include a `context` block in globals and/or a project, showing `${bot.messageId}`, `@{date}`, and a custom key.

- [ ] **7.2** Add an example `.ccpa/hooks/context.mjs` in the examples directory demonstrating a simple hook.

---

## Phase 8 — Lint & typecheck

- [ ] **8.1** Run `pnpm run lint:fix` — fix any Biome issues.
- [ ] **8.2** Run `pnpm run build` — fix any TypeScript errors.
