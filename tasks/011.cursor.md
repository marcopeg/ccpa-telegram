...
Add support for OpenAI "cursor" CLI engine:
...
Cursor works very similarly to Copilot for session management (`--continue` flag) but you have to research the codex CLI for other params.
...
Options
  ...
  -h --help                                  Print usage.
  --add-mcp <json>                           Adds a Model Context Protocol server definition to the user profile, or workspace or folder when used with --mcp-workspace. Accepts JSON input in the form '{"name":"server-name","command":...}'
