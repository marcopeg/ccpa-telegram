...
## Phase 1 — Add `chokidar` Dependency ✅ COMPLETED
...
## Phase 2 — Command Loader (`src/bot/commands/loader.ts`) ✅ COMPLETED
...
## Phase 3 — Command Watcher (`src/bot/commands/watcher.ts`) ✅ COMPLETED
...
## Phase 4 — Runtime Command Dispatch (`src/bot/handlers/text.ts`) ✅ COMPLETED
...
## Phase 5 — Wire into `startBot` (`src/bot.ts`) ✅ COMPLETED
...
## Phase 6 — Example Command File ✅ COMPLETED
...
- [x] Watcher is running and watching both directories after bot start
- [x] Watcher reacts to `add` events (new files), not just `change`
- [x] Debounce fires after Claude finishes writing (300ms is sufficient)
- [x] `loadCommands` re-imports with cache-busting (`?t=Date.now()`)
- [x] `bot.api.setMyCommands` is called with the full updated list
- [x] Telegram clients receive the updated command list (usually within seconds)
